...
#pragma omp parallel for schedule (dynamic,1)
        for (unsigned int synth=0; synth<n_synths; synth++)
        {
            ...
            else if (curr_det.compare("ORB")==0)
            {
                ...
                cv::Ptr<cv::ORB> CurrentDetector = cv::ORB::create(det_par.ORBParam.nfeatures,
            ...
            else if (curr_det.compare("FAST")==0)
            {
                ...
                temp_img1.pixels.convertTo(CharImage,CV_8U);
                cv::FAST(CharImage,keypoints_1,det_par.FASTParam.threshold,
            ...
            else if (curr_det.compare("BRISK")==0)
            {
                ...
                temp_img1.pixels.convertTo(CharImage,CV_8U);
                cv::Ptr<cv::BRISK> CurrentDetector = cv::BRISK::create(det_par.BRISKParam.thresh,
            ...
            else if (curr_det.compare("STAR")==0)
            {
                ...
                temp_img1.pixels.convertTo(CharImage,CV_8U);
                cv::Ptr<cv::xfeatures2d::StarDetector> CurrentDetector = cv::xfeatures2d::StarDetector::create(det_par.STARParam.maxSize,
...
#pragma omp critical
                ...
                else if (curr_desc.compare("ORB") == 0) //ORB
                {
                    ...
                    cv::Ptr<cv::ORB> CurrentDescriptor = cv::ORB::create(det_par.ORBParam.nfeatures,
                                                             det_par.ORBParam.scaleFactor,
                                                             det_par.ORBParam.nlevels,
                                                             det_par.ORBParam.edgeThreshold,
                                                             det_par.ORBParam.firstLevel,
                ...
                else if (curr_desc.compare("FREAK") == 0) //FREAK
                {
                    ...
                    //  const double mrSizeORB = 3.0;
                    cv::Ptr<cv::xfeatures2d::FREAK> CurrentDescriptor = cv::xfeatures2d::FREAK::create(desc_par.FREAKParam.orientationNormalized,
